...
export default class RegisterProfessional extends Component {
  constructor(props) {
    super(props);
  ...
  saveUser = async () => {
    if (
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/user/createprofessional`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        swal({
          ...
          text: "Please wait for admin approval. You'll receive an email once it's approved.",
          icon: "success",
          button: "OK",
        }).then(() => {
